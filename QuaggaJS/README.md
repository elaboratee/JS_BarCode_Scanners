...
Работа алгоритма локализации состоит из нескольких этапов. Сначала исходное изображение может быть уменьшено в два раза по ширине и высоте для ускорения обработки. Затем выполняется бинаризация изображения с помощью метода Оцу для отделения полос штрих-кода от фона. Далее изображение делится на сетку патчей, каждый из которых анализируется. После этого патчи группируются по схожести угловых направлений, так как одномерные штрих-коды состоят из параллельных линий. Затем определяются области, содержащие наибольшее количество патчей, и на основе этих областей строятся bounding boxes, которые определяют расположение штрих-кодов на изображении. Если на каком-либо из этапов не удается обнаружить достаточно данных, локализация завершается.

## Содержание
1. [Работа локализатора](#работа-локализатора)
...
## Как запустить
Для использования необходимо скопировать все файлы из [папки](../QuaggaJS) в одну директорию на компьютере. Возможно несколько вариантов запуска, они рассмотрены ниже.
### VS Code + Live Server
1. Откройте директорию со скопированными файлами в VS Code
2. Установите расширение `Live Server` в магазине расширений
3. Кликните правой кнопкой мыши на файле `live_w_locator.html`
4. Выберите опцию `Open with Live Server`
5. Приложение запустится в браузере по умолчанию
### Python HTTP Server
1. Откройте командную строку
2. В командной строке перейдите в директорию со скопированными файлами
3. Запустите HTTP-сервер:
```python -m http.server```
5. Теперь скопированные файлы доступны в браузере по адресу:
```http://localhost:8000/```
6. Выберите файл `live_w_locator.html`
